...
# ============================================================
# 2. Terminal Output Helpers (Clean UI)
# ============================================================
...
def row_skewness(x):
    """Calculates distribution asymmetry (3rd moment)."""
...
def row_kurtosis(x):
    """Calculates distribution tailedness (4th moment)."""
    mean, std = np.mean(x, axis=1, keepdims=True, dtype=np.float32), np.std(x, axis=1, keepdims=True, dtype=np.float32) + 1e-7
    return np.mean(((x - mean) / std) ** 4, axis=1, dtype=np.float32)

def row_autocorr_lag(x, lag):
    """Calculates signal correlation with itself at a specific delay."""
...
def row_zero_crossings(x):
    """Counts how often the centered signal crosses zero."""
    c = x - np.mean(x, axis=1, keepdims=True, dtype=np.float32)
    return np.sum((c[:, :-1] * c[:, 1:]) < 0, axis=1).astype(np.float32)

def row_local_extrema(x):
    """Counts peaks and troughs by looking at directional changes."""
    d = np.diff(x, axis=1)
...
def add_deep_features(features, names, x, prefix, autocorr_lags=None):
    """Percentiles, moments, and shape descriptors."""
...
def add_temporal_blocks(features, names, x, prefix, n_blocks=4):
    """Splits signal into sub-windows to capture changes over time."""
...
# ============================================================
# 5. Core Feature Extraction (Routing signals to math functions)
# ============================================================
def extract_features(X_raw, feature_columns):
    features, names = [], []
    
    # --- A. Slice Matrix into Modalities ---
    ...
    # --- B. Cardiac Features (BVP, VPG, APG) ---
    add_deep_features(features, names, bvp, "bvp", [16, 20, 24, 28, 32, 38, 44, 50, 58, 66, 76, 86, 96])
    ...
    # --- C. Motion Features (Accelerometer & Jerk) ---
    ...
    add_basic_features(features, names, np.diff(acc_sq, axis=1), "jerk")

    # --- D. Stress/Electrodermal Features (EDA) ---
    add_deep_features(features, names, eda, "eda", [1, 2, 4])
    add_temporal_blocks(features, names, eda, "eda")

    # --- E. Cross-Modal & Noise Proxies ---
...
def main():
    ...
    start = time.perf_counter()
    print_step(2, 6, "Extracting mathematical & temporal features...")
    ...
    # Add tol=0.001 to help it finish smoothly
