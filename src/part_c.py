...
HUBER_ALPHA = 1e-5
HUBER_EPSILON = 1.25  # Slightly lowered to be more robust against motion outliers
HUBER_MAX_ITER = 1000
...
# ============================================================
# NEW PHYSICS/MATH FEATURES
# ============================================================

def add_spectral_features(features, names, bvp, acc_sq):
    """Concept 1 & 3: FFT Mathematical Orthogonalization and Spectral Entropy"""
    # 1. BVP Spectral (64 Hz over 10s = 640 samples)
    bvp_fft = np.abs(np.fft.rfft(bvp, axis=1))
    bvp_power = bvp_fft ** 2
    ...
    # Extract dominant BVP frequency peak
    bvp_peak_idx = np.argmax(bvp_hr_fft, axis=1)
    bvp_peak_bpm = bvp_hr_freqs[bvp_peak_idx] * 60.0
    add_feature(features, names, bvp_peak_bpm, "bvp_spectral_peak_bpm")
    
    # Calculate Spectral Entropy (Information Theory concept of signal chaos)
    p_bvp = bvp_power / (np.sum(bvp_power, axis=1, keepdims=True) + 1e-10)
    bvp_entropy = -np.sum(p_bvp * np.log2(p_bvp + 1e-10), axis=1)
    add_feature(features, names, bvp_entropy, "bvp_spectral_entropy")
    
    # 2. ACC Spectral (32 Hz over 10s = 320 samples)
    acc_fft = np.abs(np.fft.rfft(acc_sq, axis=1))
    ...
    add_feature(features, names, acc_peak_bpm, "acc_spectral_peak_bpm")
    
    # 3. Spectral Orthogonality (Difference between expected HR peak and MA peak)
    add_feature(features, names, np.abs(bvp_peak_bpm - acc_peak_bpm), "bvp_acc_spectral_diff")
...
def extract_features(X_raw, feature_columns):
    ...
    # --------------------------------------------------------
    # Inject New Math/Physics Concepts Here
    # --------------------------------------------------------
...
def main():
    ...
    # Added tol=1e-3 to significantly improve convergence speed
