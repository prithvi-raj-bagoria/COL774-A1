...
def main():
    ...
    predictions_path, weights_path = sys.argv[3], sys.argv[4]
    ...
    # MEMORY CLEANUP: Delete DataFrame instantly
    del train 

    # ==========================================
    # 2. AUGMENT MATRIX AND SOLVE
    # ==========================================
    ones_train = np.ones((X_train.shape[0], 1), dtype=np.float64)
    X_train_aug = np.hstack((ones_train, X_train))
    
    # MEMORY CLEANUP: Unaugmented data no longer needed
    ...
    # MEMORY CLEANUP: Augmented matrix no longer needed
    del X_train_aug 
